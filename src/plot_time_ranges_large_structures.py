...
def compare(
    ...
    spec_sequence = subfigs[0].add_gridspec(2, num_predictions + 1, wspace=0.1, hspace=0.1)
...
def main(argv) -> None:
    ...
    for i, file in enumerate(files):
        ...
        labels.append(
            f"from z={to_redshift(
                (config.file_index_start + config.file_index_stride) / 100):.1f}")
