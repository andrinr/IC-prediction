...
Using telescopes and similar tools, we can observe the distribution of stars in the universe. Many processes on earth happen fast, sometimes even too fast for us to observe them. Meanwhile in space we have the opposite problem, the timescale is too large and everything just appears static. The universe however is anything but static. It has gone through billions of years of change, where the underlying mass distribution and the dynamics at play are ever evolving. Having a better understanding of the evolution of the universe allows us to improve the accuracy of our models and predictions. Those in turn can help us understand structures that are invisible to us, such as dark matter and dark energy.


Numerical simulations are frequently used to simulate matter and its gravitational interactions, which in turn can serve as a model of the observed universe. Such simulations rely on initial conditions that set the course of the simulation. That is, the final state of the simulation is determined by the initial conditions. Such initial conditions are commonly sampled from random matter density distributions that mirror the known properties of the distributions of the very early stages of our universe. The fundamental statistics of the density distribution of the universe are known because of observations of the Cosmic Microwave Background (CMB). The CMB is the remnant radiation from the Big Bang, and its temperature fluctuations provide a snapshot of the universe when it was only 380,000 years old. By analyzing these fluctuations, we can infer the statistical properties of the density field at that time. We can then evolve those initial density fields forward in time by discretizing mass using particles, where the gravitational effects between particles evolve the particles in time. Computing all interacting forces between all particles is computationally expensive as it scales quadratically with the number of particles. We can however leverage methods such as the multiple expansion to obtain efficient methods which scale linearly with the number of particles \citep{potter2017pkdgrav3, springel2021simulating, garrison2021abacus}.


To understand the past of the universe it would be more helpful to reverse those simulations and to go back in time. In theory, numerical solvers can be reversed by simply changing the sign of the forces. However, using obersvational techniques we are unable to recover the complete velocity vectors. Therefore we cannot find the propper initial conditions for such an inverse simulation. This is further complicated by the highly nonlinear dynamics affecting numerical stability \citep{jasche2013bayesian}. Instead of reversing the solver, we can recover an approximate initial condition for a specific final state. That is instead of starting with a random realization of those known statistical properties of the early universe, we start with an initial condition that evolves into structures roughly following the desired distribution. Specifically we want a function capable of generating a conditional initial condition, where the condition is the distribution of the final simulation state.


Previously proposed methods often rely on iterative optimization methods. Here a random guess for the initial condition is fed into the forward simulation. The resulting final state is then compared with the desired distribution, and an objective function is evaluated. We can then use gradient free methods, such as bayesian optimization, to approximate the multidimensional surface of the objective by continuously probing it at different points \citep{jasche2013bayesian}. In that case, each point on the surface represents a specific initial condition and the associated value of the objective function. We can then exploit our knowledge about the surface to find good initial conditions. Approximating the loss surface for high-dimensional problems, can become computationally infeasible \citep{djolonga2013high} due to operations that are involved in the process. If we implement the forward solver in a differentiable manner, we can deploy a gradient based local optimization method that is computationally more efficient and scales well to large problems. We can also use hybrid approaches that use gradients to increase the convergence of bayesian optimization \citep{wang2014elucid}.


As an alternative approach, we examine the application of deep learning methods to predict the conditional initial conditions. More precisely, we train a neural operator that is able to generate initial conditions for a given final state. The operator is trained on a simulation dataset that is generated using the N-Body solver PKDGRAV3 \citep{potter2017pkdgrav3}. We assign the masses of the particles from the simulation on a density field that is discretized on an equidistant regular grid. This enables us to apply well-studied convolutional-based neural network architectures. To improve the accuracy of the predictions, we train unique neural networks on specific sub time intervals of the entire simulation duration. We then chain those networks together to form a single operator that is able to predict the initial conditions for a final state. Such autoregressive training schemes suffer from exploding and vanishing gradients, which we mitigate with staged training pipelines. In the early training stage, each neural network is trained independently of the others. Later on, the neural networks are chained together and learn to mitigate errors introduced by the previous networks. We achieve good results on the prediction of the initial conditions, by using the Fourier Neural Operator (FNO) \citep{li2020fourier} as the underlying architecture. The FNO is a neural network that is capable of learning mappings between function spaces, making it well-suited for problems involving partial differential equations (PDEs). The FNO leverages the Fourier transform to efficiently capture long-range dependencies in the data, and inherently respects the periodic boundary conditions that are present in the simulation data. By analyzing the power spectrum correlation between the generated density fields and the ground truth, we can conclude that our model is able to predict structures well above the nonlinear threshold of \(\mathbf{k} = 1\). Furthermore, we can observe that capabilities of the model are way beyond a linear interpolation of the initial conditions.

At the time of writing this thesis, leveraging deep learning for predictions across time steps in N-Body simulations is a completely novel field and presents exciting new challenges and opportunities. Our results not only highlight the feasibility of using deep learning for conditional initial conditions, but highlight the potential of deep learning to replace and complement numerical solvers in the future.
