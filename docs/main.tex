...
We can use telescopes and similar tools to observe the distribution of stars in the universe. Many processes on Earth happen too quickly for us to observe. However, because the relevant physical processes unfold over cosmological timescales, the universe appears static to us. The universe has undergone billions of years of change, during which the underlying mass distribution and dynamics have continuously evolved. Thanks to observations of the Cosmic Microwave Background (CMB), we know the fundamental statistics of the universe's density distribution. The CMB is remnant radiation from the Big Bang, and its temperature fluctuations provide a snapshot of the universe when it was only 380,000 years old. By analyzing these fluctuations, we can infer the statistical properties of the density field at that time.

We can use our knowledge of the statistical properties of the universe's density distribution to initialize numerical simulations of matter and its gravitational interactions.
These simulations can then serve as a model of the observed universe. The initial conditions are random noise realizations that are correlated using a correlation kernel derived from the power spectrum of the early universe. We then evolve the initial density field forward in time by discretizing its mass with particles. Next, we compute the gravitational effects between particles and use the forces to evolve the particles over time. However, computing interactions between all particles is computationally expensive, as naive implementations scale quadratically. Methods such as multipole expansions can reduce this cost and scale linearly, thus providing a more efficient approach \citep{potter2017pkdgrav3, springel2021simulating, garrison2021abacus}.

Although forward modeling is well established, the inverse problem—recovering the initial condition from the final matter distribution—remains a significant challenge. In theory, numerical solvers can be reversed by changing the sign of the forces. However, we are unable to recover the complete velocity vectors using observational techniques. Therefore, we cannot determine the proper initial conditions for an inverse simulation. This is further complicated by the highly nonlinear dynamics that affect numerical stability. \citep{jasche2013bayesian}. Rather than reversing the solver, we can derive an approximate initial condition from a specific final state. In other words, rather than starting with a random realization of the known statistical properties of the early universe, we start with an initial condition that evolves into structures that roughly follow the desired distribution. Specifically, we want a function capable of generating a conditional initial condition where the condition is the distribution of the final simulation state. Previously proposed methods rely on iterative optimization. In this method, a random guess for the initial condition is fed into the forward simulation. The resulting final state is compared with the desired distribution, and an objective function is evaluated. We can then use gradient-free methods, such as Bayesian optimization, to approximate the multidimensional surface of the objective function by probing it continuously at different points \citep{jasche2013bayesian}.In that case, each point on the surface corresponds to a particular initial condition and its associated objective function value. We can use our knowledge of the surface to find good initial conditions. However, approximating the loss surface for high-dimensional problems can become computationally infeasible \citep{djolonga2013high} due to the operations involved in the process. Implementing the forward solver in a differentiable manner allows us to deploy a gradient-based local optimization method that is computationally efficient and scalable to large problems. We can also use hybrid approaches that employ gradients to improve the convergence of Bayesian optimization \citep{wang2014elucid}.


In this thesis, we propose an alternative approach that uses deep learning. Specifically, we train a neural operator that predicts initial density fields based on a given final state. We train the operator on a simulation dataset generated using the N-body solver PKDGRAV.3 \citep{potter2017pkdgrav3}. We assign masses to particles in a simulation based on a discretized density field on an equidistant, regular grid. This allows us to apply well-studied, convolutional-based neural network architectures. To improve the accuracy of the predictions, we train unique neural networks on specific sub-time intervals of the simulation's total duration. Then, we chain those networks together to form a single operator that can predict the initial conditions for the final state. However, such autoregressive training schemes suffer from exploding and vanishing gradients, which we mitigate with staged training pipelines. During the initial training stage, each neural network is trained independently. Later, the networks are connected and learn to reduce errors introduced by previous networks. Using the Fourier Neural Operator (FNO), we achieve good results in predicting the initial conditions. \citep{li2020fourier} as the underlying architecture. The FNO is a neural network capable of learning mappings between function spaces, making it ideal for problems involving partial differential equations (PDEs). The FNO uses the Fourier transform to efficiently capture long-range dependencies in the data and naturally considers the periodic boundary conditions present in the simulation data. By analyzing the correlation between the power spectrum of the generated density fields and the ground truth, we can conclude that our model can predict structures well above the nonlinear threshold of \(\mathbf{k} = 1\). Furthermore, we can see that the model's capabilities far exceed linear interpolation of the initial conditions.

At the time this thesis was written, using deep learning to make predictions across time steps in N-body simulations was an entirely new field that presented exciting new challenges and opportunities. Our results highlight the feasibility of using deep learning for conditional initial conditions and demonstrate its potential to replace and complement numerical solvers in the future.
...
In this thesis, we explore the feasibility of using Fourier neural operators (FNOs) to predict initial dark matter simulation conditions directly from their final states. Specifically, our work addresses the inverse problem of reconstructing the primordial density field at a high redshift of $z=49$ from evolved distributions at lower redshifts. Our approach uses deep learning techniques to overcome the computational and numerical difficulties of traditional gradient-based and gradient-free optimization methods, which are commonly used for constrained initial condition prediction. We conducted the following experiments in this thesis:
...
\begin{itemize}
    ...
    \item  Time Interval Sensitivity: Predictions degraded with longer time intervals between input and output. While autoregressive training helped mitigate this issue, it introduced trade-offs between accuracy, memory usage, and training time.
...
\begin{itemize}
    ...
    \item Training Stability: Long time-step training risked overfitting or divergence Multistep predictions showed promise but further increase the memory requirements and training time. However, once those challenges are overcome, more steps could be used to increase the stability of the training. Feature engineering and a smart choice of time step sizes could potentially enable using a single network for all time steps.
...
Our work demonstrates the potential of neural operators to accelerate cosmological inference, enabling rapid exploration of initial conditions consistent with observed structures. While challenges remain, this approach complements traditional optimiization methods, and could even be used as better intial guesses for such methods. Overall, this thesis lays the groundwork for future research at the intersection of machine learning and cosmology.
\newpage
