...
Numerical simulations are frequently used to simulate matter and its gravitational interactions, which in turn can serve as a model of the observed universe. Such simulations rely on an initial conditions that set the course of the simulation. That is the final state of the simulation is determined by the initial conditions. Such initial conditions are sampled from random matter density distributions that mimic the distributions of the very early stages of our universe. The fundamental statistics of the density distribution of the universe are known because of observations of the Cosmic Microwave Background (CMB). The CMB is the remnant radiation from the Big Bang, and its temperature fluctuations provide a snapshot of the universe when it was only 380,000 years old. By analyzing these fluctuations, scientists can infer the statistical properties of the density field at that time. We can evolve such density fields forward in time by discretizing mass using particles, where the gravitational effects between particles are computed and used to evolve particles in time. To compute all interacting forces between all particles is computationally expensive as it scales quadratically with the number of particles. We can however leverage methods such as the multiple expansion to obtain efficent methods which scale linearly with the number of particles \citep{potter2017pkdgrav3, springel2021simulating, garrison2021abacus}.

To understand the past of the universe it would be more helpful to reverse those simulations and to go back in time. In theory, the numerical solvers could be reversed by simply changing the sign of the forces. However, using obersvational techniques we are unable to recover the complete velocity vector of matter. Therefore we cannot find the complete initial conditions for such an inverse simulation. This is further complicated by the highly nonlinear dynamics affecting numerical stability \citep{jasche2013bayesian}. Instead of reversing the sovler, we can recover an approximate initial conditions for a specific final state. That is instead of starting with a random realization of those known statistical properties of the early universe, we start with an initial condition that evolves into structures that roughly follow the desired distribution. Specifically we want a functions capabale of generating a conditional initial condition, where the condition is the distribution of the final simulation state.

Previously proposed methods often rely on iterative optimization methods. Here a random guess for the initial conditions is fed into the forward simulation. The resulting final state is then compared with the desired distribution, and an objective function is evaluated. We can then use gradient free methods, such as beysian optimization, to approximate the mutlidimensional surface of the objective by continously probing it at different points \citep{jasche2013bayesian}. In that case, each point on the surface represent a specific initial condition and the associated value of the objective function. We can then exploit our knowledge about the surface to find good initial conditions. Approximating the loss surface for high-dimensional problems, can become computationally infeasible \citep{djolonga2013high} due to operations that are involved in the process. If we implement the forward solver in a differentiable manner, we can deploy a gradient based local optimization methods that is computationally more efficent and scales well to large problems. We can also use hybrid approaches that use gradients to increase the convergence of bayesian optimization \citep{wang2014elucid}.

As an alternative approach, we examine the application of deep learning methods to predict the conditional initial conditions. More precisely, we train a neural operator that is able to generate intial conditions for a given final state. The operator is trained on a simulation dataset that is generated using the N-Body solver PKDGRAV3 \citep{potter2017pkdgrav3}. We assign the masses of the particles from the simulation on a density field that is discretized on an equidistant regular grid. This enables us to apply well-studied convolutional-based neural network architectures. To improve the accuracy of the predictions, we train unique neural networks on specific sub time intervals of the entire simulation duration. We then chain those networks together to form a single operator that is able to predict the initial conditions for a final state. Such autoregressive training schemes suffer from exploding and vanishing gradients, which we mitigate with staged training pipelines. In the early training stage, each nerual network is trained independently of the others. Later on, the neural networks are chained together and learn to mitigate errors inroduced by the previous networks. We achieve good results on the prediction of the initial conditions, by using the Fourier Neural Operator (FNO) \citep{li2020fourier} as the underlying architecture. The FNO is a neural network that is capable of learning mappings between function spaces, making it well-suited for problems involving partial differential equations (PDEs) and other complex systems. The FNO leverages the Fourier transform to efficiently capture long-range dependencies in the data, which is particularly useful for simulating physical systems governed by PDEs. By analyzing the power spectrum of the generated density fields, we can conclude that our model is able to predict structures well above the nonlinear threshold of \(\mathbf{k} = 1\). Furthermore, we can observe that capabilities of the model are way beyond a linear interpolation of the initial conditions. 
